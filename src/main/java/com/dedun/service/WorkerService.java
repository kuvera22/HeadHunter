...
public class WorkerService {
    ...
    public WorkerResponse saveWorker(WorkerRequest workerRequest) throws HeadHunterException {
        ...
        Worker worker = new Worker(workerRequest.getLogin(), null, workerRequest.getFirstName(),
                workerRequest.getLastName(), workerRequest.getEmail());
    ...
    public List<WorkerResponse> getAll() {
        return workerRepository.findAll().stream().map(worker -> new WorkerResponse(worker.getId(),
                worker.getLogin(), worker.getFirstName(), worker.getLastName(), worker.getEmail())).collect(Collectors.toList());
    }
